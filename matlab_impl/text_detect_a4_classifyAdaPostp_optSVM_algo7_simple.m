function text_detect_a4_classifyAdaPostp_optSVM_algo7_simple(path, name, feat, rules, useSVM)
...
path_token = '../../../../results_Ku_vs_Newmann/Ku_Result/';

for reverse = 0:1
    
    chk_txt = [in_mat_path in_mat_file '_reverse_' num2str(reverse) '.mat'];
    if ~exist(chk_txt, 'file')
        continue;
    end
    
%     chk_txt = [path_token idx_label '_reverse_' num2str(reverse) '.txt'];
    chk_txt = [path_token sprintf('%03d', name.TESTING_IMG_IDX) '_r' num2str(reverse) '.png'];
    if exist(chk_txt, 'file')
        continue;
    else
        fid = fopen(chk_txt,'w');
    ...
    out_img_path = util_changeFn(out_img_path, 'cd _mkdir', [in_mat_file '_ER_candidate_img_' num2str(useSVM) '_r' num2str(reverse) time_label]);
    ...
    if c2>0
        ...
        s = [out_img_path '../' sprintf('%03d', name.TESTING_IMG_IDX) '_r' num2str(reverse) '.png'];
        s = chk_txt;
        imwrite(I_accum, s, 'png');
        s = [out_img_path '[' sprintf('%03d', name.TESTING_IMG_IDX) ']_r' num2str(reverse) '_c1_' num2str(c1) '_c2_' num2str(c2)  '.png'];
        imwrite(I_accum, s, 'png');
    end
